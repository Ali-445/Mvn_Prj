# Rapport TPJavaPipeline

## 1 - Récupération de Token après avoir créer un compte dans ngrok

<img width="1918" height="959" alt="webhookP1" src="https://github.com/user-attachments/assets/d3ff3c61-07b2-4a1e-85dd-e2f9d3e5d43f" />

## 2 - Lancement d'un Coneteneur Docker avec l'image de ngrok  

<img width="1918" height="734" alt="webhookP2" src="https://github.com/user-attachments/assets/ce0639e4-3da0-4124-99a2-35d6808124e5" />

## 3 - Configuration de Webhook dans github avec le domaine générer par ngrok

<img width="1901" height="850" alt="webhookP3" src="https://github.com/user-attachments/assets/57753113-cec5-4f22-b888-eba7bad9b025" />

<br><br>

<img width="1901" height="707" alt="webhookP4" src="https://github.com/user-attachments/assets/0553e639-eaca-4e39-8679-1bc3890f1750" />

<br><br>

<img width="1900" height="860" alt="webhookP5" src="https://github.com/user-attachments/assets/520910b3-dc45-42c3-9092-7c414c4c4d06" />

## 4 - Vérification de logs de Jenkins pour voir le ping de github 

<img width="1883" height="903" alt="webhookP6" src="https://github.com/user-attachments/assets/ec72c820-40b4-47b7-99f0-22a783599704" />

## 5 - Vérification du premier build fait automatiquement par Jenkins après le scan de github repo

<img width="1909" height="902" alt="webhookP7" src="https://github.com/user-attachments/assets/ecee8787-7be0-40d5-8821-2fa71afc85da" />

## 6 - L'ajout d'un Commentaire dans un fichier ,puis pusher le fichier pour Trigger le Pipeline

<img width="1744" height="976" alt="webhookP8" src="https://github.com/user-attachments/assets/6afcbcd2-ab51-4efc-ad5a-eb821202ae8d" />

<br><br>

<img width="1554" height="813" alt="webhookP9" src="https://github.com/user-attachments/assets/375f9b92-8dfa-4c0b-9736-feaeebf7bbb4" />


## 7 - Vérification que jenkins a Recu la notification de push et a effectuer le build

<img width="1913" height="906" alt="webhookP10" src="https://github.com/user-attachments/assets/dc4ea17b-ec40-4cf9-84cc-3527abbc15cf" />

<br><br>

<img width="1903" height="895" alt="webhookP11" src="https://github.com/user-attachments/assets/f7316919-0d6d-4f70-89ef-d6707f762598" />

<br><br>

<img width="1887" height="899" alt="webhookP12" src="https://github.com/user-attachments/assets/76212b81-f98e-43cc-9896-fb1c59b2f859" />







